...
function vote(answerId, type, btn) {
  ...
  .then(data => {
    ...
    // Silently handle failures without showing alerts
  })
  .catch(() => {
...
function attachCommentEditDeleteHandlers(questionId) {
  console.log('Attaching comment handlers...');
  ...
  document.querySelectorAll('.edit-comment-btn').forEach(btn => {
    ...
    btn.onclick = function() {
      ...
      // The button is inside comment-actions div, which is inside comment-card
      const commentActionsDiv = btn.closest('.comment-actions');
      if (!commentActionsDiv) {
        console.error('Comment actions div not found');
        return;
      }
      
      const commentDiv = commentActionsDiv.parentElement;
      if (!commentDiv || !commentDiv.classList.contains('comment-card')) {
        console.error('Comment card not found');
        return;
      }
      
      console.log('Found comment div:', commentDiv);
      const contentDiv = commentDiv.querySelector('.comment-content');
      if (!contentDiv) {
        console.error('Comment content div not found');
        return;
      }
      ...
      editForm.innerHTML = `
        <textarea class="edit-comment-textarea" style="width:100%;">${oldContent}</textarea>
      ...
      const saveBtn = editForm.querySelector('.save-edit-comment-btn');
      const cancelBtn = editForm.querySelector('.cancel-edit-comment-btn');
      
      if (!saveBtn || !cancelBtn) {
        console.error('Save or cancel buttons not found in edit form');
        return;
      }
      ...
      saveBtn.onclick = function() {
        ...
        console.log('Saving comment:', { id: commentId, content: newContent });
...
// Add a MutationObserver to handle dynamically added comments
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (questionCommentsContainer) {
    observer.observe(questionCommentsContainer, { childList: true, subtree: true });
  }
  if (answersContainer) {
